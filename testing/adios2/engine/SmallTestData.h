...
SmallTestData generateNewSmallTestData(SmallTestData input,
                                       int step, int rank, int size)
{
    ...
    {   // Make sure that data is within the range
        ...
        input.I8[i] = clip(static_cast<char>(input.I8[i] +
                           static_cast<char>(jump)),
                           std::numeric_limits<char>::min(),
                           std::numeric_limits<char>::max());
        input.SI8[i] = clip(static_cast<signed char>(input.SI8[i] +
                           static_cast<signed char>(jump)),
                           std::numeric_limits<signed char>::min(),
                           std::numeric_limits<signed char>::max());
        input.I16[i] = clip(static_cast<short>(input.I16[i] +
                           static_cast<short>(jump)),
                           std::numeric_limits<short>::min(),
                           std::numeric_limits<short>::max());
        input.I32[i] = clip(static_cast<int>(input.I32[i] +
                           static_cast<int>(jump)),
                           std::numeric_limits<int>::min(),
                           std::numeric_limits<int>::max());
        input.I64[i] = clip(static_cast<long>(input.I64[i] +
                           static_cast<long>(jump)),
                           std::numeric_limits<long>::min(),
                           std::numeric_limits<long>::max());

        input.U8[i] = clip(static_cast<unsigned char>(input.U8[i] +
                           static_cast<unsigned char>(jump)),
                           std::numeric_limits<unsigned char>::min(),
                           std::numeric_limits<unsigned char>::max());
        input.U16[i] = clip(static_cast<unsigned short>(input.U16[i] +
                           static_cast<unsigned short>(jump)),
                           std::numeric_limits<unsigned short>::min(),
                           std::numeric_limits<unsigned short>::max());
        input.U32[i] = clip(static_cast<unsigned int>(input.U32[i] +
                           static_cast<unsigned int>(jump)),
                           std::numeric_limits<unsigned int>::min(),
                           std::numeric_limits<unsigned int>::max());
        input.U64[i] = clip(static_cast<unsigned long int>(input.U64[i] +
                           static_cast<unsigned long int>(jump)),
                           std::numeric_limits<unsigned long int>::min(),
                           std::numeric_limits<unsigned long int>::max());

        input.R32[i] = clip(static_cast<float>(input.R32[i] +
                           static_cast<float>(jump)),
                           -std::numeric_limits<float>::max(),
                           std::numeric_limits<float>::max());
        input.R64[i] = clip(static_cast<double>(input.R64[i] +
